...
import { useGetUSContactsQuery } from "../services/api";
import ListItem from "./ListItem";

const ModalUS = () => {
  const [list, setList] = useState(null);
  const [isEven, setIsEven] = useState(false);
  const { data, isSuccess } = useGetUSContactsQuery();
    ...
    <>
      <div>
        ...
        <div style={{ position: "absolute", height: "500px", width: "500px", top: "calc(50vh - 250px)", left: "calc(50vw - 250px)", backgroundColor: "white", borderRadius: "20px", padding: "20px" }}>
          ...
          US
          <ul>{list ? 
          (isEven ? list.filter((color) => color.id % 2 === 0).map((v, i) => <ListItem key={`list-${Math.random()}`} list={v} />) 
          : list.map((v, i) => <ListItem key={`list-${Math.random()}`} list={v} />)) : "Loading"}</ul>
